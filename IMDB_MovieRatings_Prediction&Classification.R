...
str(movie2.df)
t(t(colnames(movie2.df)))
movie2.df<-movie2.df[,-c(20)]
...
#compute Euclidean Distance - movie.norm.df as we took only numerical variables
...
# We got ratio for between_ss/total_ss to be 33.3%
# let's see if we need higher % to achieve heterogeneity between clusters and homogeneity within cluster.
...
ggcorrplot(correlations,  type = "upper", outline.col = "white", lab = TRUE)
...
sum$adjr2 # based on adj R2 we are going to select 7 predictors
sum$cp
...
str(movie1.df)
movie.data<- dummy.data.frame(movie1.df[ ,-c(1,4,6,10,12,13,14)], sep =".")
movie_nn.data <- as.data.frame(apply(movie.data,2,rescale))


names(movie_nn.data)<-make.names(names(movie_nn.data),unique = TRUE)
set.seed(211)
train1.rows<-sample(rownames(movie_nn.data), dim(movie_nn.data)[1]*0.5)
valid1.rows<-sample(setdiff(rownames(movie_nn.data), train1.rows), dim(movie_nn.data)[1]*0.3)
test1.rows<-setdiff(rownames(movie_nn.data), union(train1.rows, valid1.rows))
train.df<-movie_nn.data[train1.rows,];valid.df<-movie_nn.data[valid1.rows,];test.df<-movie_nn.data[test1.rows,]


library(neuralnet)
########################################################################################

# Creating Neural Network with 1 hidden layer and 2 nodes

movienn<- neuralnet(imdb_score ~ ., data = train.df,linear.output=TRUE, hidden = 2)
movienn$weights
# plot network
plot(movienn, rep="best")
# display predictions
pred.train<-compute(movienn,train.df)

##on validation data#################
pred.valid <- compute(movienn,valid.df)

# on test data###############
pred.test <- compute(movienn,test.df)

##accuracy on the training data
pred.train.imdb <- pred.train$net.result*(max(movie1.df$imdb_score)-min(movie1.df$imdb_score))+min(movie1.df$imdb_score)
train.imdb <- (train.df$imdb_score)*(max(movie1.df$imdb_score)-min(movie1.df$imdb_score))+min(movie1.df$imdb_score)
#head(data.frame(train.df$imdb_score,train.imdb))
#head(pred.train.imdb)
###RMSE on train data
library(ModelMetrics)
rmse(train.imdb,pred.train.imdb)
#RMSE: 0.8115884

##accuracy on validation data
pred.valid.imdb <- pred.valid$net.result*(max(movie1.df$imdb_score)-min(movie1.df$imdb_score))+min(movie1.df$imdb_score)
valid.imdb <- (valid.df$imdb_score)*(max(movie1.df$imdb_score)-min(movie1.df$imdb_score))+min(movie1.df$imdb_score)
#head(data.frame(valid.df$imdb_score,valid.imdb))
##RMSE on validation data
rmse(valid.imdb,pred.valid.imdb)
# RMSE: 0.8367603

# MAE on validation data
mae(valid.imdb,pred.valid.imdb)

# MAE: 0.631832

##accuracy on test data
pred.test.imdb <- pred.test$net.result*(max(movie1.df$imdb_score)-min(movie1.df$imdb_score))+min(movie1.df$imdb_score)
test.imdb <- (test.df$imdb_score)*(max(movie1.df$imdb_score)-min(movie1.df$imdb_score))+min(movie1.df$imdb_score)
#head(data.frame(valid.df$imdb_score,valid.imdb))

##RMSE on Test data
rmse(test.imdb,pred.test.imdb)
# RMSE: 0.8238042

# MAE on Test data
mae(test.imdb,pred.test.imdb)
# MAE: 0.630374

########################################################################################
# Creating different models with different hidden layers


movienn_1_3<- neuralnet(imdb_score ~ ., data = train.df,linear.output=TRUE, hidden = 3)
movienn_1_4<- neuralnet(imdb_score ~ ., data = train.df,linear.output=TRUE, hidden = 4)
movienn_2_2<- neuralnet(imdb_score ~ ., data = train.df,linear.output=TRUE, hidden = c(2,2))
movienn_2_4<- neuralnet(imdb_score ~ ., data = train.df,linear.output=TRUE, hidden = c(2,4))



##on validation data#################
pred.valid_1_3 <- compute(movienn_1_3,valid.df)
pred.valid_1_4 <- compute(movienn_1_4,valid.df)
pred.valid_2_2 <- compute(movienn_2_2,valid.df)
pred.valid_2_4 <- compute(movienn_2_4,valid.df)


#scaling back to original data

pred.valid.imdb_1_3 <- pred.valid_1_3$net.result*(max(movie1.df$imdb_score)-min(movie1.df$imdb_score))+min(movie1.df$imdb_score)
pred.valid.imdb_1_4 <- pred.valid_1_4$net.result*(max(movie1.df$imdb_score)-min(movie1.df$imdb_score))+min(movie1.df$imdb_score)
pred.valid.imdb_2_2 <- pred.valid_2_2$net.result*(max(movie1.df$imdb_score)-min(movie1.df$imdb_score))+min(movie1.df$imdb_score)
pred.valid.imdb_2_4 <- pred.valid_2_4$net.result*(max(movie1.df$imdb_score)-min(movie1.df$imdb_score))+min(movie1.df$imdb_score)


##RMSE on validation data
rmse(valid.imdb,pred.valid.imdb_1_3)# RMSE: 0.8356946 for 1 hidden layer with 3 nodes
rmse(valid.imdb,pred.valid.imdb_1_4)# RMSE: 0.8467044
rmse(valid.imdb,pred.valid.imdb_2_2)# RMSE: 0.8344388
rmse(valid.imdb,pred.valid.imdb_2_4)# RMSE: 0.8351012


mae(valid.imdb,pred.valid.imdb_1_3)# MAE: 0.6344301
mae(valid.imdb,pred.valid.imdb_1_4)# MAE: 0.6347333
mae(valid.imdb,pred.valid.imdb_2_2)# MAE: 0.6314718
mae(valid.imdb,pred.valid.imdb_2_4)# MAE: 0.6281281



# We analyzed that if we go on increasing the number of hidden layers and their corresponding nodes,
# the accuracy on the validation data doesn’t improve. Among all these 5 Neural Networks, we find the
# accuracy metrics to be best for the Neural Network with 1 hidden layer and 2 nodes. 


## Accuracy metrics on validation data
# RMSE: 0.8367603
# MAE: 0.631832
...
confusionMatrix(pred1,valid1.data$rating) # Accuracy 67.95
testpred <- predict(multinom.fit, newdata = test1.data)
confusionMatrix(testpred,test1.data$rating) # accuracy 70.14


#commented as it is taking long time
############################################
...
# k -10 and accuracy is 66.48% for valid data. So running for k=10 to check confusion matrix

movieknn.pred.def <- knn(train = knntrain.norm[,c(1:20)], test = knnvalid.norm[,1:20], 
                     cl = knntrain.norm[,21], k = 10)
confusionMatrix(movieknn.pred.def, valid1.data[, 13])
...
movieknn1 <- knn(train = knntrain1.norm[,c(1:15)], test = knnvalid1.norm[,c(1:15)], 
                cl = knntrain1.norm[,16], k = 20) #we are predicting "rating", so cl = knntrain1.norm[,16] using k=10
...
plot(movieaccuracy1.df[,2], xlab = "K-Value", ylab = "Accuracy in predicting rating", ylim = c(0.60,0.69))

# As we got best accuracy for K = 20, we are now checking the accuracy on test data to compare with other models

movieknn2 <- knn(train = knntrain1.norm[,c(1:15)], test = knntest1.norm[,c(1:15)], 
                 cl = knntrain1.norm[,16], k = 20)

confusionMatrix(movieknn2, knntest1.norm[,16]) # Accuracy 69.18
